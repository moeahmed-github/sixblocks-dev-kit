---
name: cc-fact-check
description: Use when documentation mentions a Claude Code command, slash command, CLI flag, keyboard shortcut, config file path, settings key, hook event name, MCP behaviour, agent frontmatter field, or any product behaviour — and before any page containing a terminal command is marked ready.
---

# Claude Code Fact Check

Nothing about Claude Code reaches a reader unverified. A beginner has no way
...
2. Verify each one by **running it**, or by reading **official documentation**,
   ...
   acceptable sources.
3. Record the Claude Code version verified against and stamp it in the page
   footer: "Verified against Claude Code vX.Y, <month year>."
4. Flag anything likely to change, and add a note telling the reader how to
   check current behaviour themselves (`/help`, `claude --version`, the docs
   link).
...
| "The feature obviously exists" | Inventing a plausible-sounding feature is the single most damaging thing documentation can do. |
...
- It works on your OS and no one checked the other one
